...
function generateDecoderSkeleton(templateName: string, channels: ChannelInfo[]): string {
  const channelParsers = channels.map((ch, index) => {
    const offset = index * 2; // Placeholder offset
    return `  // ${ch.name} (Channel ${ch.channel})
  // TODO: Adjust byte offset and parsing logic based on your device's payload format
  // const ${ch.name.toLowerCase().replace(/[^a-z0-9]/g, '_')}Raw = buffer.readInt16BE(${offset});
  sensors.push({
    channel: ${ch.channel},
    type: '${ch.type}',
    unit: '${ch.unit}',
    value: 0, // TODO: Parse from buffer
    name: '${ch.name}'
  });`;
  }).join('\n\n');
...
/**
 * Decoder for: ${templateName}
 *
 * Generated by myDevices CLI
 *
 * Channels:
${channels.map((ch) => ` *   - ${ch.channel}: ${ch.name} (${ch.type}, ${ch.unit})`).join('\n')}
 *
 * Decoder API:
 *   Decoder.payload.data        - Raw payload (hex string)
 *   Decoder.payload.fport       - LoRaWAN fport
 ...
 *   Decoder.error(message)      - Report error
 */
try {
  ...
  // console.log('Raw payload:', Decoder.payload.data);
...
export function createTemplatesCommands(): Command {
  ...
  // --------------------------------------------------------------------------
  // templates scaffold-decoder
  // --------------------------------------------------------------------------
  templates
    .command('scaffold-decoder')
    ...
    .action(async (templateId: string, options: { output?: string }) => {
      const spinner = ora('Fetching template...').start();
      try {
        // Fetch the template with channels
        const template = await apiGet<DeviceTemplate>(`${getTemplatesPath()}/${templateId}`);

        if (!template.channels || template.channels.length === 0) {
          spinner.stop();
          error('Template has no capabilities. Add capabilities first.');
          process.exit(1);
        }

        spinner.text = 'Fetching datatype information...';

        // Fetch datatype info for each channel to get the payload (type)
        const channelInfo: Array<{
          channel: string;
          name: string;
          type: string;
          unit: string;
        ...
        for (const channel of template.channels) {
          const datatypeId = channel.data_types_id;
          let type = 'unknown';
          let unit = 'unknown';

          if (datatypeId) {
            // Get datatype to find the payload (type)
            const datatypes = await apiGet<ApiResponse<Datatype>>('/v1.0/things/datatypes', {
              limit: 100,
              page: 0,
            });
            const datatype = datatypes.rows?.find((d) => d.id === datatypeId);
            if (datatype) {
              type = datatype.payload;
            }

            // Get default unit from channel data or datatype properties
            if (channel.data?.units && channel.data.units.length > 0) {
              const defaultUnit = channel.data.units.find((u) => u.default) || channel.data.units[0];
              unit = defaultUnit.payload || 'unknown';
            }
          }
        ...
        // Generate decoder skeleton
        const decoderSource = generateDecoderSkeleton(template.name, channelInfo);

        if (options.output) {
          writeFileSync(options.output, decoderSource);
          success(`Decoder skeleton written to ${options.output}`);
        } else {
          console.log(decoderSource);
        }
      } catch (err) {
        spinner.stop();
        error(err instanceof Error ? err.message : 'Failed to generate decoder skeleton');
        process.exit(1);
      }
    });
